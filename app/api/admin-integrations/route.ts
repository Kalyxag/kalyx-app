...
export async function GET(req: Request) {
  ...
  const tid = await tenantId(a.admin, slug)
  if (!tid) return NextResponse.json({ error: 'Mandant nicht gefunden.' }, { status: 404 })

  const r = await a.admin.from('tenant_integrations')
...
export async function POST(req: Request) {
  ...
  if (body.mode === 'test') {
    ...
    await a.admin.from('tenant_integrations')
  ...
  const up = await a.admin.from('tenant_integrations').upsert(rows, { onConflict: 'tenant_id,integration_key' })
  if (up.error) {
