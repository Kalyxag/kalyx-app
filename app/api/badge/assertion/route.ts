...
export async function GET(req: Request) {
  ...
  try {
    ...
    const { data } = await admin
      .from('certificates')
      .select('cert_number,title,recipient_name,issued_at,status,course_id,user_id,content_hash,expires_at')
      .eq('cert_number', nr)
      .maybeSingle()
    ...
    // Widerrufen → Spezifikation: 410 + revoked
    if (c.status === 'widerrufen') {
      return jsonLd({ '@context': OB_CONTEXT, id: assertionId, revoked: true }, 410)
    }
    // Abgelaufen → ebenfalls als widerrufen/ungültig melden (410), mit Grund.
    if (c.expires_at && new Date(c.expires_at) <= new Date()) {
      return jsonLd({ '@context': OB_CONTEXT, id: assertionId, revoked: true, revocationReason: 'Rezertifizierung erforderlich (abgelaufen).' }, 410)
    }
    ...
    // Ablaufdatum nach Open-Badges-Standard
    if (c.expires_at) assertion.expires = new Date(c.expires_at).toISOString()
    // Inhalts-Fingerabdruck mittragen: bindet das Badge an die geprüfte Fassung.
    if (c.content_hash) assertion.contentHash = c.content_hash

    return jsonLd(assertion)
  } catch {
