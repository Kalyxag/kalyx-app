...
//
// Gefuehrter Setup-Assistent. Aktiv: Firmenprofil, Abteilungen/Rollen, Compliance.
// Kurse, Branding, Mitarbeitende folgen als naechste Runden.
...
const ACTIVE_MAX = 2 // Schritte 0..2 sind in dieser Runde bedienbar
...
const RECERT: { v: string; l: string }[] = [
  { v: 'jaehrlich', l: 'Jaehrlich' }, { v: 'halbjaehrlich', l: 'Halbjaehrlich' },
...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (comp) {
        ...
        setEsig(!!c.esignature_required)
        setRecert(c.recert_interval_default || 'jaehrlich')
  ...
  async function saveProfile() {
    ...
    setSavingProfile(false); setStep(1)
  }
  ...
  async function saveCompliance() {
    ...
    setSavingComp(false); setDone(true)
  }
  ...
  const chip = (on: boolean): React.CSSProperties => ({ fontSize: 13, fontWeight: 600, padding: '8px 14px', borderRadius: 8, cursor: 'pointer', border: `1.5px solid ${on ? GREEN : '#DDD9D0'}`, color: on ? '#fff' : NAVY, background: on ? GREEN : '#fff' })
  const toggleRow: React.CSSProperties = { display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 14px', background: '#F5F4EF', borderRadius: 8, marginBottom: 12 }
  ...
  if (done) {
    return (
      <div style={page}><div style={shell}><div style={card}>
        ...
        <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 26, color: NAVY, marginBottom: 12 }}>Grundlagen gespeichert</h1>
        <p style={{ fontSize: 15, color: GRAY, lineHeight: 1.6, marginBottom: 8 }}>
          Firmenprofil, <strong style={{ color: NAVY }}>{depts.length}</strong> Abteilung(en), <strong style={{ color: NAVY }}>{roles.length}</strong> Rolle(n) und der Compliance-Rahmen ({frameworks.length} Regelwerk{frameworks.length === 1 ? '' : 'e'}) sind gespeichert.
        </p>
        <p style={{ fontSize: 14, color: GRAY, lineHeight: 1.6, marginBottom: 24 }}>
          Als Naechstes folgen: Mitarbeitende einladen, Kurse zuweisen und Branding. Diese Schritte bauen wir in den naechsten Runden an dieselbe Stelle.
        </p>
    ...
    <div style={page}>
      <div style={shell}>
        ...
        <div style={card}>
            ...
            <>
              ...
              <div style={row2}>
                <div>
                  ...
                  <select style={input} value={sector} onChange={e => setSector(e.target.value)}>{SECTORS.map(s => <option key={s.v} value={s.v}>{s.l}</option>)}</select>
                </div>
                <div>
                  <label style={label}>Unternehmensgroesse</label>
                  <select style={input} value={size} onChange={e => setSize(e.target.value)}><option value="">bitte waehlen</option>{SIZES.map(s => <option key={s} value={s}>{s} Mitarbeitende</option>)}</select>
                </div>
              </div>
              <div style={row2}>
                <div>
                  <label style={label}>Land</label>
                  <select style={input} value={country} onChange={e => setCountry(e.target.value)}>{COUNTRIES.map(c => <option key={c} value={c}>{c}</option>)}</select>
                </div>
                <div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
                {LANGS.map(l => <button key={l.v} type="button" onClick={() => toggleLang(l.v)} style={chip(langs.includes(l.v))}>{l.l}</button>)}
              </div>
              ...
              <button style={{ ...btn, opacity: savingProfile ? 0.7 : 1 }} onClick={saveProfile} disabled={savingProfile}>{savingProfile ? 'Speichern …' : 'Speichern und weiter'}</button>
            </>
          )}
          ...
          {step === 1 && (
            <>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                ...
                <select style={{ ...input, marginBottom: 0, flex: '1 1 130px' }} value={newRoleDept} onChange={e => setNewRoleDept(e.target.value)}><option value="">keine Abteilung</option>{depts.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}</select>
                <select style={{ ...input, marginBottom: 0, flex: '1 1 120px' }} value={newRoleAccess} onChange={e => setNewRoleAccess(e.target.value)}>{ACCESS.map(a => <option key={a.v} value={a.v}>{a.l}</option>)}</select>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 22 }}>
                ...
                <button style={btn} onClick={async () => { setBusy(true); await supabase.from('onboarding_state').update({ step_departments: true, updated_at: new Date().toISOString() }).eq('tenant_id', tenantId); await audit('departments_completed', 'onboarding_state', tenantId); setBusy(false); setStep(2) }} disabled={busy}>Weiter zu Compliance</button>
              </div>
            </>
          )}
          ...
          {step === 2 && (
            <>
              <h1 style={{ fontFamily: 'Georgia, serif', fontSize: 24, color: NAVY, marginBottom: 4 }}>Compliance-Rahmen</h1>
              <p style={{ fontSize: 13.5, color: GRAY, marginBottom: 8 }}>Welche Regelwerke gelten fuer euch? KALYX richtet Kurse, Rezertifizierung und Audit-Trail danach aus.</p>
              <p style={{ fontSize: 12.5, color: GOLD, marginBottom: 22 }}>Hinweis: Diese Auswahl beschreibt euren Compliance-Bedarf. Im Pilot werden Daten in der EU gespeichert; voll Schweizer-souveraenes Hosting ist das Ziel.</p>

              <label style={label}>Relevante Regelwerke</label>
              <div style={{ display: 'flex', gap: 8, marginBottom: 22, flexWrap: 'wrap' }}>
                {FRAMEWORKS.map(f => <button key={f} type="button" onClick={() => toggleFramework(f)} style={chip(frameworks.includes(f))}>{f}</button>)}
              </div>

              <div style={toggleRow}>
                <input type="checkbox" checked={esig} onChange={e => setEsig(e.target.checked)} style={{ marginTop: 2, accentColor: GREEN }} />
                <div><div style={{ fontSize: 14, fontWeight: 600, color: NAVY }}>Elektronische Signatur bei Abschluss</div><div style={{ fontSize: 12.5, color: GRAY }}>Lernende bestaetigen den Kursabschluss rechtsgueltig per E-Signatur.</div></div>
              </div>
              <div style={toggleRow}>
                <input type="checkbox" checked={residency} onChange={e => setResidency(e.target.checked)} style={{ marginTop: 2, accentColor: GREEN }} />
                <div><div style={{ fontSize: 14, fontWeight: 600, color: NAVY }}>Datenhaltung CH/EU bestaetigt</div><div style={{ fontSize: 12.5, color: GRAY }}>Wir nehmen zur Kenntnis, dass Daten im Pilot in der EU liegen.</div></div>
              </div>

              <div style={row2}>
                <div>
                  <label style={label}>Rezertifizierung (Standard)</label>
                  <select style={input} value={recert} onChange={e => setRecert(e.target.value)}>{RECERT.map(r => <option key={r.v} value={r.v}>{r.l}</option>)}</select>
                </div>
                <div>
                  <label style={label}>Aufbewahrung (Monate)</label>
                  <input style={input} type="number" min={0} value={retention} onChange={e => setRetention(parseInt(e.target.value || '0', 10))} />
                </div>
              </div>

              {msg && <p style={{ color: '#C0392B', fontSize: 13, margin: '6px 0 12px' }}>{msg}</p>}
              <div style={{ display: 'flex', gap: 12, marginTop: 10 }}>
                <button style={btnGhost} onClick={() => setStep(1)}>Zurueck</button>
                <button style={{ ...btn, opacity: savingComp ? 0.7 : 1 }} onClick={saveCompliance} disabled={savingComp}>{savingComp ? 'Speichern …' : 'Speichern und abschliessen'}</button>
              </div>
            </>
          )}
        </div>
